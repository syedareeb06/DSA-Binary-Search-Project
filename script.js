document.getElementById("startSearch").addEventListener("click", function () {
    ...
    const array = arrayInput.split(",").map((num) => parseInt(num.trim())).filter((num) => !isNaN(num)).sort((a, b) => a - b);
    ...
    if (!array.length || isNaN(target)) {
        stepsDiv.innerHTML = "<p>Please enter a valid sorted array and a target number!</p>";
        return;
    }
...
function visualizeBinaryTree(array, container) {
    ...
    const drawTree = (array, level = 0, leftOffset = container.offsetWidth / 2) => {
        if (!array.length) return;
        ...
        if (leftArray.length > 0) {
            ...
            drawTree(leftArray, level + 1, leftChildX);
        }
        ...
        if (rightArray.length > 0) {
            ...
            drawTree(rightArray, level + 1, rightChildX);
        }
    };
